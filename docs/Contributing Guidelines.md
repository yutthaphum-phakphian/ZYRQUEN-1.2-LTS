...
1. **Branching Strategy**: Create a feature or fix branch off `main` using structured naming:
   ```bash
   git checkout -b feature/pqc-audit-extension
   # or
   git checkout -b fix/telemetry-latency-drift
   ```
2. **Local Pre-flight Check**: Run strict type checks and test suites locally:
   ```bash
   npx tsc --noEmit
   npm test
   ```
3. **Signed Commits**: Commit changes with GPG signature enabled:
   ```bash
   git commit -S -m "feat(security): update invariant attestation logic"
   ```
4. **Security Gate Verification**: Push your branch and verify that all 7 stages of `zyrquen-security-gate.yml` pass successfully on GitHub.
