...
export const PWAInstallButton: React.FC = () => {
  ...
  if (isInstallable) {
    return (
      <button
        onClick={install}
        className="flex items-center gap-2 rounded-xl bg-emerald-600/20 px-3 py-1.5 text-xs font-mono font-bold text-emerald-300 border-emerald-500/40 shadow-sm hover:bg-emerald-600/30 transition-all shadow-[0_0_15px_rgba(16,185,129,0.15)]"
      >
    ...
    return (
      <>
        <button
          ...
          className="flex items-center gap-2 rounded-xl bg-blue-600/20 px-3 py-1.5 text-xs font-mono font-bold text-blue-300 border-blue-500/40 shadow-sm hover:bg-blue-600/30 transition-all"
        >
          <Download className="w-3.5 h-3.5" />
          INSTALL ON IOS
        </button>
        ...
        {showIOSGuide && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#07080F]/90 backdrop-blur-xl p-4">
            <div className="w-full max-w-sm rounded-2xl bg-[#0b0e1a] border-blue-500/30 p-6 shadow-2xl text-center font-sans relative overflow-hidden">
              <div className="w-12 h-12 mx-auto rounded-xl bg-blue-500/10 border-blue-500/30 flex items-center justify-center mb-4">
                <Share className="w-6 h-6 text-blue-400" />
              </div>
              <h3 className="text-lg font-bold text-white mb-2">Install on iPhone / iPad</h3>
              <div className="text-sm text-zinc-300 space-y-3 font-mono text-left bg-black/40 p-4 rounded-xl border-white/5">
