...
export const ChamberDetail: React.FC<ChamberDetailProps> = ({
  ...
  return (
    <div className="flex-1 overflow-y-auto p-5 space-y-5 bg-[#05080f] select-none">
      {/* Chamber Header Card */}
      <div className="p-4 rounded-xl bg-gradient-to-r from-slate-950 via-[#091122] to-slate-950 border border-slate-800 shadow-md">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
          <div>
            <div className="flex items-center space-x-2.5">
              <span className="font-mono-code text-xs font-bold px-2 py-0.5 rounded bg-cyan-950 text-cyan-300 border border-cyan-800">
                CHAMBER {chamber.id}
              </span>
              <span className="text-xs text-slate-400">[{chamber.category}]</span>
              <span className="text-[10px] font-mono-code px-1.5 py-0.5 rounded bg-emerald-950 text-emerald-300 border border-emerald-800/80">
                {chamber.status}
              </span>
            </div>
          ...
          <div className="flex items-center space-x-2">
            <button
              ...
              className="px-3 py-1.5 rounded-lg bg-slate-900 hover:bg-slate-800 border border-slate-700 text-xs text-slate-300 font-medium transition-all"
            >
              Open CLI Shell
            </button>
            <button
              onClick={onOpenDossier}
              className="px-3 py-1.5 rounded-lg bg-cyan-950 hover:bg-cyan-900 border border-cyan-800 text-xs text-cyan-200 font-medium transition-all"
            >
              View Legal Dossier
            </button>
          </div>
          ...
          <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 max-h-64 overflow-y-auto pr-1">
              {VERIFICATION_PHASES.map((p) => (
                <div
                  key={p.id}
                  className="p-2 rounded bg-slate-900/60 border border-slate-800 text-[11px] flex items-center justify-between"
                >
                  <div className="truncate mr-1">
                    <span className="font-mono-code text-cyan-400 font-bold mr-1.5">{p.id}</span>
                    <span className="text-slate-300">{p.name}</span>
                  </div>
                  <span className="text-[9px] font-mono-code px-1.5 py-0.2 rounded bg-emerald-950 text-emerald-400 font-bold border border-emerald-800">
                    PASS
                  </span>
                </div>
              ))}
          ...
          <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5">
              {INVARIANTS.map((inv) => (
                <div
                  key={inv.id}
                  className="p-2.5 rounded bg-slate-900/50 border border-slate-800 flex items-start justify-between space-x-2 text-xs"
                >
                  <div>
                  ...
                  <span className="text-[9px] font-mono-code px-1.5 py-0.5 rounded bg-emerald-950 text-emerald-300 border border-emerald-800/60 shrink-0">
                    {inv.status}
                  </span>
                </div>
              ))}
        ...
        <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-3">
          ...
          <div className="flex items-center space-x-2">
            <input
              ...
              className="flex-1 bg-slate-900 border border-slate-800 rounded px-3 py-1.5 text-xs text-slate-200 focus:outline-none focus:border-cyan-500"
            />
            <button
              onClick={() => handleInspectSeal(sealSearchIndex)}
              className="px-4 py-1.5 rounded bg-cyan-950 hover:bg-cyan-900 border border-cyan-800 text-xs text-cyan-200 font-semibold"
            >
              Verify Leaf Hash
            </button>
          </div>
          ...
          {inspectedSeal && (
            <div className="p-3 bg-slate-900/80 rounded border border-slate-800 font-mono-code text-xs space-y-1.5">
        ...
        <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-4">
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              className="w-full bg-slate-900 border border-slate-800 rounded px-3 py-2 text-xs text-slate-200 font-mono-code focus:outline-none focus:border-cyan-500"
            />
          </div>
        ...
        <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
            <button
              ...
              className="p-2.5 rounded bg-slate-900 hover:bg-slate-800 border border-slate-800 text-xs text-slate-300 font-mono-code"
            >
              Play 432 Hz SSoT Tone
            </button>
            <button
              onClick={() => soundFx.playPhoenixChime()}
              className="p-2.5 rounded bg-amber-950/60 hover:bg-amber-900/60 border border-amber-800/80 text-xs text-amber-300 font-mono-code"
            >
              Play Phoenix 142ms Chord
            </button>
            <button
              onClick={() => soundFx.playAlertBuzz()}
              className="p-2.5 rounded bg-rose-950/60 hover:bg-rose-900/60 border border-rose-800/80 text-xs text-rose-300 font-mono-code"
            >
          ...
          <div className="space-y-2 pt-2 border-t border-slate-800">
            ...
            <div className="flex space-x-2">
              ...
              <select
                ...
                className="bg-slate-900 border border-slate-800 rounded px-2.5 py-1.5 text-xs text-slate-300"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {TREASURY_ASSETS.map((asset) => (
              <div
                ...
                className="p-3 bg-slate-900/60 border border-slate-800 rounded-lg space-y-1.5"
              >
