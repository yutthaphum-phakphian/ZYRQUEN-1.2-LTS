...
export default function ForensicAuditPlaybackDashboard() {
  ...
  const [merkleRoot, setMerkleRoot] = useState<string>('0x909ab8f1c3d2e4a5b6c7d8e9f0a1b2c3d4e5f6a7');
  const [lastReplayTime, setLastReplayTime] = useState<string>(new Date().toISOString());
  const [apiEndpoint, setApiEndpoint] = useState<string>('http://localhost:4000/api/v1/replay/verify');
  ...
  const runLiveReplayAudit = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setMerkleRoot(data.merkleRoot || '0x909ab8f1c3d2e4a5b6c7d8e9f0a1b2c3d4e5f6a7');
        setLastReplayTime(data.timestampUTC || new Date().toISOString());
        setIsLiveConnected(true);
      } else {
        console.warn('Backend API returned non-200 status, using fallback.');
        setIsLiveConnected(false);
      }
    } catch (err) {
      console.warn('Backend connection failed, executing fallback simulated replay:', err);
      setIsLiveConnected(false);
      // Simulate quick fallback replay animation
    ...
    <div className="bg-[#0F172A] border border-cyan-900/50 rounded-2xl p-6 text-gray-100 font-sans space-y-6 shadow-2xl">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-800 pb-4">
        ...
        <div className="flex items-center space-x-3">
          <div className={`px-3 py-1 rounded-lg text-xs font-mono font-bold flex items-center gap-2 border ${
            isLiveConnected
            ...
            <span>{isLiveConnected ? 'CONNECTED: server.ts (Port 4000)' : 'FALLBACK MODE (Local Deterministic)'}</span>
          </div>

          <button
            onClick={runLiveReplayAudit}
            disabled={loading}
            className="flex items-center space-x-2 px-5 py-2 rounded-xl text-sm font-bold bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 text-white shadow-lg shadow-cyan-500/20 transition-all disabled:opacity-50"
          >
            {loading ? (
              <>
                ...
                <span>Executing Replay (server.ts)...</span>
              </>
            ) : (
              <>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between text-xs font-mono text-gray-400 px-2">
          <span>12-STAGE REPLAY WATERFALL SEQUENCE</span>
      ...
      <div className="border-t border-slate-800 pt-3 flex flex-wrap items-center justify-between text-xs font-mono text-gray-500">
        ...
        <div>MERKLE ROOT: <span className="text-cyan-400">{merkleRoot}</span></div>
        <div className="text-emerald-400">ISO/IEC 27037 FORENSICS COMPLIANT</div>
      </div>
    </div>
  );
}
