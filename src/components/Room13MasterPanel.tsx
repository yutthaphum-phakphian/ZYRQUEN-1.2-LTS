...
export const Room13MasterPanel: React.FC<Room13MasterPanelProps> = ({
  ...
  return (
    <div className="space-y-6 font-mono text-zinc-300">
      ...
      <div className="p-6 sm:p-8 rounded-[28px] bg-gradient-to-br from-blue-950/40 via-[#071224]/95 to-black border-blue-500/40 backdrop-blur-xl relative overflow-hidden shadow-2xl">
        ...
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div className="space-y-3">
            <div className="flex flex-wrap items-center gap-2">
              <span className="px-3 py-1 rounded-full bg-blue-500/15 text-blue-300 border-blue-500/40 text-xs font-bold flex items-center gap-1.5 shadow-[0_0_12px_rgba(59,130,246,0.25)]">
          ...
          <div className="flex flex-wrap lg:flex-col items-center lg:items-end gap-3 shrink-0">
            <button
              ...
              disabled={isPinging}
              className="px-4 py-2.5 rounded-xl bg-gradient-to-r from-blue-600/80 to-cyan-600/80 hover:from-blue-500 hover:to-cyan-500 text-white font-bold text-xs flex items-center gap-2 shadow-lg shadow-blue-500/20 border-blue-400/40 transition-all transform hover:-translate-y-0.5"
            >
      ...
      {/* Sub-Tabs */}
      <div className="flex flex-wrap items-center gap-2 p-1.5 rounded-2xl bg-[#0a0d16] border-blue-500/20">
        <button
          onClick={() => {
            playTone(600, 0.04);
          ...
          className={`px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-2 ${
            activeTab === 'bft-mesh'
              ? 'bg-blue-500/20 text-blue-200 border-blue-500/40 shadow-[0_0_12px_rgba(59,130,246,0.2)]'
      ...
      {activeTab === 'bft-mesh' && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {GLOBAL_NODES.map((node) => (
            <div
              key={node.id}
              className="p-5 rounded-2xl bg-[#0a0d1a] border-blue-500/20 hover:border-blue-500/50 transition-all space-y-3"
            >
        ...
        <div className="p-6 rounded-2xl bg-[#0a0d1a] border-cyan-500/20 space-y-4">
          ...
          <div className="space-y-2">
            {GLOBAL_NODES.map((n) => (
              <div
                key={n.id}
                className="p-3.5 rounded-xl bg-black/40 border-white/5 flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs"
              >
                <div>
      ...
      {/* Tab 3: Latency Chart */}
      {activeTab === 'latency-matrix' && (
        <div className="p-6 rounded-2xl bg-[#0a0d1a] border-blue-500/20 space-y-4">
          <h3 className="text-base font-bold text-white flex items-center gap-2">
