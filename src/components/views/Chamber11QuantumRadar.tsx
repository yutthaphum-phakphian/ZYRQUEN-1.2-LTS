...
export const Chamber11QuantumRadar: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <MerkleWaveCanvas baselineDrift={baselineDrift} isGlitching={isGlitching} />
    </div>
  );
};

export const MerkleWaveCanvas: React.FC<{ baselineDrift: number, isGlitching: boolean }> = ({ baselineDrift, isGlitching }) => {
  const canvasRef = useRef<HTMLCanvasElement | null>(null);
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      ctx.strokeStyle = isGlitching ? 'rgba(239, 68, 68, 0.8)' : 'rgba(16, 185, 129, 0.8)';
      ctx.lineWidth = 2;

      for (let x = 0; x < width; x++) {
        const primaryWave = Math.sin((x + offset) * 0.05) * 20;
        const secondaryWave = Math.sin((x + offset * 1.5) * 0.1) * 10;
        const drift = baselineDrift * 50;
        const glitchOffset = isGlitching ? (Math.random() - 0.5) * 15 : 0;
        
        const y = height / 2 + primaryWave + secondaryWave + drift + glitchOffset;
        ctx.lineTo(x, y);
      }
      ctx.stroke();
  ...
  const handleExport = () => {
    const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify({ 
      ...
      telemetry: "wave_data", 
      baselineDrift, 
      type: "SIGNED_JSON_ARTIFACT", 
      signature: "0xMerkleWaveHash123..." 
    }, null, 2));
    const dlAnchorElem = document.createElement('a');
    dlAnchorElem.setAttribute("href", dataStr);
    dlAnchorElem.setAttribute("download", "wave_telemetry.json");
    dlAnchorElem.click();
  };

  return (
    <div className={`relative w-full h-48 border border-cyan-500/20 bg-[#070a12] rounded-xl overflow-hidden mt-4 shadow-lg ${isGlitching ? 'animate-pulse' : ''}`}>
      <div className="absolute top-2 left-3 flex items-center gap-2 z-10 pointer-events-none">
        <Activity className="w-4 h-4 text-emerald-400" />
        <span className="text-[10px] font-mono font-bold text-emerald-400 uppercase tracking-widest">Merkle Waveform & SSoT Drift Telemetry</span>
      </div>
      <canvas ref={canvasRef} className="w-full h-full block" width={800} height={200} />
      <button onClick={handleExport} className="absolute bottom-2 right-2 px-3 py-1.5 bg-black/60 hover:bg-cyan-500/20 text-cyan-300 text-[10px] font-mono font-bold rounded border border-cyan-500/30 flex items-center gap-1.5 transition-colors">
         <Download className="w-3.5 h-3.5" /> Export Artifact
      </button>
    </div>
  );
};
