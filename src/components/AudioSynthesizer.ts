...
export const getAudioSynthesizerStatus = (): AudioSynthesizerStatus => {
  const profile = AUDIO_PROFILES.find((p) => p.id === activeProfileId) || AUDIO_PROFILES[1];
  return {
    state: audioCtx ? audioCtx.state : 'uninitialized',
    isCarrierActive: sovereignOscillator !== null,
    isCryoHumActive: cryoHumGain !== null,
...
export const restartAudioSynthesizer = async (): Promise<boolean> => {
  try {
    stopAmbientProfile();
    toggleCryoHum55_110(false);
    ...
    console.error('Failed to restart audio synthesizer:', err);
    return false;
  }
};

export class EntropyAudioSynthesizer {
  private ctx: AudioContext | null = null;
  private osc: OscillatorNode | null = null;
  private gainNode: GainNode | null = null;
  private isRunning: boolean = false;
  private baseFreq: number = 110; // A2 pitch base
  ...
  public init() {
    ...
    this.osc.frequency.setValueAtTime(this.baseFreq, this.ctx.currentTime);
    this.gainNode.gain.setValueAtTime(0.015, this.ctx.currentTime); // Subtle background audio
  ...
  public start() {
    this.init();
    if (this.ctx && this.osc && !this.isRunning) {
  ...
  public updateEntropy(entropyLevel: number) {
    if (!this.ctx || !this.osc || !this.isRunning) return;
    
    // Clamp entropy [0, 1] and map to frequency pitch modulation (110Hz to 880Hz)
    const normalized = Math.max(0, Math.min(1, entropyLevel));
    const targetFrequency = this.baseFreq + normalized * 770; 

    // Smooth frequency transition over 100ms
    this.osc.frequency.setTargetAtTime(targetFrequency, this.ctx.currentTime, 0.1);
  }

  public stop() {
    if (this.osc && this.isRunning) {
      this.osc.stop();
...
export const entropyAudioSynthesizer = new EntropyAudioSynthesizer();
